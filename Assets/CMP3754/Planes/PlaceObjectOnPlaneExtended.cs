...
public class PlaceObjectOnPlaneExtended : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (Input.touchCount > 0)
        {
            ...
            //if (EventSystem.current.IsPointerOverGameObject(touch.fingerId)) //this should work, but doesn't.
    ...
    private bool UnhighlightObject()
    {
...
//    public Material unhighlightMat;
//    GameObject highlighted;
//    bool anyHighlightted = false;

    }


    private bool HighlightObject(GameObject obToHightlight)
    {
    ...
    public bool DeleteHighlighted()
    {
        GameObject hl = highlighted;
        if (!UnhighlightObject()) return false;
        if (!spawnedObjects.Contains(hl)) return false;
        spawnedObjects.Remove(hl);
        Destroy(hl);
        return true;
    }

    public int QueryHighlighted()
    {
        if (highlighted) QueryCount++;
        return QueryCount;
    }
